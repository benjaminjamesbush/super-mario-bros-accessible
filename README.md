...
But unmodified SMB1 doesn't let that experience happen. The very first barrier is the title screen — "press Start to play" is a learned gaming convention, not something intuitive. A player with intellectual disabilities may never get past it, or lose interest before figuring out how to begin. Beyond the title screen, bottomless pits and enemies kill Mario and force the player to restart. The timer kills Mario after 400 ticks — a sudden death with no obvious visible cause. Springboards require precisely timed button presses. Castle mazes loop endlessly without the right path memorized. Each of these is a source of frustration. For a player with intellectual disabilities, that frustration can mean the difference between engagement and abandonment.
